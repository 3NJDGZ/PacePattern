...
class authRoutes(baseView):
    def __init__(
        self, flaskApp, cachingSystem: CachingSystem, garminUserClient: GarminUserClient
    ):
    ...
    def _setupRoutes(self):
        ...
        def login():
            ...
            if request.method == "POST":
                ...
                try:
                    ...
                    usrFullName = (
                        self.__garminUserClient.getGarminClient().get_full_name()
                    )
                    ...
                    return render_template(
                        "login.html", usrFullName=usrFullName, error=error
                    )
            else:
                return render_template(
                    "login.html", usrFullName=usrFullName, error=error
        ...
        def logged_in(usrFullName):
            if self.__garminUserClient.getGarminClient() is not None:
                activityData = self.__cachingSystem.getActivityData(
                    garminClient=self.__garminUserClient.getGarminClient()
                )
            else:
                activityData = []
