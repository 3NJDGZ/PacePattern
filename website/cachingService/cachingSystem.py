    ...
    def getActivityData(self, garminClient: Garmin):
        ...
        for activityID in activityIDs:
            ...
            if activity is not None:
                if (
                    self.__cacheClient.getDataFromKey(
                        self.__cacheClient.createKey(activityID)
                    )
                    is None
                    and self.__dbClient.retrieveRunningData(activityID) is None
                ):
                    ...
                    hrStreamData = garminClient.get_heart_rates(formattedDate)[
                        "heartRateValues"
                    ]
                    ...
                    predictedIntensity = kmeans_predictor.predict(
                        distanceRan, elevationGain, elapsedTime, averageHR
                    )
                    dataToBeAdded = {
                        "athleteID": athleteID,
                    ...
                    self.__dbClient.insertRunningData(
                        athleteID,
                        activityID,
                        distanceRan,
                        elapsedTime,
                    ...
                    self.__cacheClient.insertJSONData(
                        600, self.__cacheClient.createKey(activityID), dataToBeAdded
                    )
                else:
                    # check redisDB cache
                    dataToBeAdded = self.__cacheClient.getDataFromKey(
                        self.__cacheClient.createKey(activityID)
                    )
                        ...
                        activityDataToDisplay.append(
                            {
                                ...
                                "predictedIntensity": dataToBeAdded[
                                    "predictedIntensity"
                                ],
